...
public class VisualizerImpl implements Visualizer {
    ...
    @Override
    public void visualize(List<SimulationObject> objects, long runTime, long currentIterationNumber) {
        Iteration iteration = new Iteration(currentIterationNumber, objects.size(), objects, runTime);
        latestShapes = createShapes(iteration);
        visualizationPanel.draw(latestShapes);
    }

    @Override
    public void visualize(long currentIterationNumber, int numberOfObjects, long runTime, String[] ids, boolean[] deleted,
                          double[] positionX, double[] positionY, double[] radiuses, int[] colors) {
        latestShapes = createShapes(currentIterationNumber, numberOfObjects, runTime, ids, deleted, positionX, positionY, radiuses, colors);
        visualizationPanel.draw(latestShapes);
    }
    ...
    private List<ShapeWithColorAndText> createShapes(Iteration iteration) {
        ...
        return createShapes(iteration.getCycle(), iteration.getNumberOfObjects(), iteration.getRunTime(), ids, new boolean[iteration.getNumberOfObjects()],
                            positionX, positionY, radiuses, colors);
    }

    private List<ShapeWithColorAndText> createShapes(long currentIterationNumber, int numberOfObjects, long runTine, String[] ids, boolean[] deleted,
                                                     double[] positionX, double[] positionY, double[] radiuses, int[] colors) {
        ...
        if (C.getShowTimeAndIteration()) {
            shapes.addAll(createInfo(currentIterationNumber, properties.getSecondsPerIteration(), numberOfObjects, runTine));
        }
        return shapes;
    }

    private Collection<? extends ShapeWithColorAndText> createInfo(long iteration, Number secondsPerIteration, int objectsCount, long runTime) {
        ...
        ShapeWithColorAndText info2 = new ShapeWithColorAndText(
                new Rectangle2D.Double(0, 60, 100, 40), BLUE, "Simulation Time: "
                + secondsToHumanReadable(secondsPerIteration.multiply(New.num(iteration)).doubleValue()));

        ShapeWithColorAndText info3b = new ShapeWithColorAndText(
                new Rectangle2D.Double(0, 100, 100, 40), BLUE, "Time: "
                + String.format("%,.6f", (double)runTime/(1000.0*1000.0*1000.0)));

        ShapeWithColorAndText info3 = new ShapeWithColorAndText(
                new Rectangle2D.Double(0, 100, 100, 40), BLUE, "Execution Time:  "
                + secondsToHumanReadable(runTime/(1000.0*1000.0*1000.0)));

        ShapeWithColorAndText info4 = new ShapeWithColorAndText(
                new Rectangle2D.Double(0, 140, 100, 40), BLUE, "Objects: " + objectsCount);
